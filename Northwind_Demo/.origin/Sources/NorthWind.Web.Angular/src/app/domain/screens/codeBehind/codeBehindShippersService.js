...
export default class CodeBehindShippersService{
	constructor(
		//--  custom dependencies
		//-- /custom dependencies
		$translate, stringFormat, enumsService, domainManagementService, shipperWizardService, wizardManagementService) {
		"ngInject";
		Object.assign(this, {$translate, stringFormat, enumsService, domainManagementService, shipperWizardService, wizardManagementService});
    }

	//CreateNew
	blockRegion1CreateNew1 (declaration, contextState) {
        const options = {
            service: this.shipperWizardService,
            commitOnClose: true,
			onAfterOpen: () => {
	...
	blockRegion1Delete1 (declaration, contextState) {
	   ...
	   return this.domainManagementService.delete('ShippersApi', keys);
	}
